...
function normalizeLineStart(prepared, start) {
	...
	return normalizeLineStartChunkIndex(prepared, cursor) < 0 ? null : cursor;
}
...
function measureAnalysis(analysis, font, includeSegments, wordBreak, letterSpacing) {
	...
	function pushMeasuredTextSegment(text, kind, start, wordLike, allowOverflowBreaks) {
		...
		if (allowOverflowBreaks && wordLike && text.length > 1) {
			...
			pushMeasuredSegment(text, width, lineEndFitAdvance, lineEndPaintAdvance, kind, start, getSegmentBreakableFitAdvances(text, textMetrics, cache, emojiCorrection, fitMode), spacingGraphemeCount);
			return;
		}
