...
- `src/lib/`: reusable layout-only core parsing, run layout, scaling, hit-testing, seeded randomness, fallback SVG, and exported types.
- `src/plugins/`: non-core canvas renderer and typed render plugins.
...
- `src/plugins/renderer.ts`: canvas renderer for `MaskRenderPlan`; draws static runs directly and materializes glyphs only when plugins request it.
- `src/plugins/hoverColor.ts`: hover color plugin.
- `src/plugins/glitch.ts`: glitch animation plugin and deterministic glyph substitution helpers.
- `src/plugins/typing.ts`: tested sample typing plugin.
- `src/lib/glyphLayout.ts`: compatibility glyph APIs, Pretext measurement helpers, and glyph spacing.
...
- Hover/glitch are plugin concerns. They materialize glyphs only for affected runs, or all runs while global glitch is active.
...
- Prefer the `MaskRenderPlan` path for new behavior. Keep `src/lib` layout-only; put renderer/plugins outside core.
- Keep legacy glyph APIs exported for compatibility, but do not route demo work through them.
...
- Animation should stay separate from core layout. Use `src/plugins` renderer/plugin hooks instead of relayouting for hover, typing, or glitch.
